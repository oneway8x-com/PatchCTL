          ...
          events: [
            {
              ...
              details: { reason: "Add English summaries" },
            },
          ],
          nextCursor: null,
        },
...
test("sends the reviewed revision and refreshes after a human decision", async ({
  page,
}) => {
  await page.goto(`/patches/${id}`);
  const decision = page.waitForRequest((request) =>
    request.url().endsWith("/decision"),
  );
  await page
...
test("does not expose approval to agent credentials even with claimed review permission", async ({
  ...
  await expect(
    page.getByText(
      "Human review is required. Agent credentials cannot approve or apply patches.",
    ),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: /Approve/ })).toHaveCount(0);
});

test("shows immutable relation labels alongside stable target IDs", async ({
  page,
}) => {
...
test("shows a queue entry with the exact proposed record count", async ({
  page,
}) => {
  await page.goto("/patches");
  await expect(
    page.getByRole("heading", { name: "Content patches" }),
  ).toBeVisible();
  await expect(
...
test("labels schedule instants as UTC and leaves activation to the consuming application", async ({
  page,
}) => {
  ...
  await page.goto(`/patches/${id}`);
  await expect(
    page.getByText("2026-09-01T08:00:00.000Z", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText(
      "Schedule instant · UTC (Z). Activation is handled by the consuming application.",
    ),
  ).toBeVisible();
});
test("distinguishes null/empty, renders content safely and paginates all records", async ({
  page,
}) => {
  await page.goto(`/patches/${id}`);
  await expect(page.getByTestId("record-diff")).toHaveCount(10);
  await expect(
    page.getByText("Null (no value)", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText("Empty string", { exact: true }).first(),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole("heading", { name: "Record 11", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText("Records 11–20 of 50", { exact: true }),
  ).toBeVisible();
  await page.screenshot({
    path: "./.patchctl-results/review.png",
    fullPage: true,
  });
});
