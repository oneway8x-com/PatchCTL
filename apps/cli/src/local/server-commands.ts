...
import { syncWithClient } from "./source-sync.js";
...
export async function runServerCommand(
  ...
  try {
    ...
    if (
      command === "login"
        ? !(rest.length === 2 && rest[0] === "--server")
        : command !== "submit" || rest.length !== 0
    )
      throw new LocalError(
        "INVALID_INPUT",
        "Use login --server ORIGIN or submit.",
      );
    const directory = configDirectory(env),
      config = await readConfig(directory),
      tenantId = config.currentTenant;
    if (!tenantId || !config.tenants[tenantId])
      throw new LocalError(
    ...
    if (command === "login") {
      ...
      if (
        ...
        throw new LocalError(
          "INVALID_TOKEN",
          "Use a scoped local-client token created by a human in the review UI.",
        );
      const connectionId = z.string().uuid().parse(actor.connectionIds[0]);
      local.server = {
        url: new URL(rest[1]).origin,
      ...
      const discovered = await withDatabase(secret, discoverResources);
      const synchronized = await syncWithClient(local, discovered, client);
      if (!env.PATCHCTL_TOKEN)
        await credentials.set(`${tenantId}/server-token`, token);
      else
        warnings.push(
          "Using PATCHCTL_TOKEN from the environment; it was not persisted.",
        );
      ...
      stdout.write(
        JSON.stringify({
          ...
          discoveredResources: synchronized.discoveredResources,
          schemaVersion: synchronized.source.schemaVersion,
          schemaChanged: synchronized.changed,
          configurationUrl: synchronized.configurationUrl,
          warnings,
        }) + "\n",
      );
      return 0;
    ...
    const actor = await client.actor();
    if (
      actor.tenantId !== server.tenantId ||
      !actor.connectionIds?.includes(server.connectionId)
    )
      throw new LocalError(
        "TENANT_MISMATCH",
        "Token does not match the configured Tenant and connection.",
      );
    const { secret, warnings } = await databaseCredential(
      credentialReference(tenantId, local),
      credentials,
    ...
    if (command !== "submit")
      throw new LocalError("INVALID_INPUT", "Unknown server command.");
    const patch = await withDraftLock(directory, tenantId, async (save) => {
      const draft = await readDraft(directory, tenantId);
      if (!local.databaseId || draft.databaseId !== local.databaseId)
        throw new LocalError(
          "DATABASE_CHANGED",
          "Recreate the patch for the current connection.",
        );
      ...
      if (draft.status !== "DRAFT") {
        ...
        if (fingerprint(frozen.server) !== fingerprint(server))
          throw new LocalError(
            "SERVER_CHANGED",
        ...
        proposal = await withDatabase(secret, async (db) => {
          const discovered = await discoverResources(db);
          await syncWithClient(local, discovered, client);
          const effective = await client.effectiveSourceSchema(
            server.connectionId,
          );
          const serverResources = applyEffectiveSchema(discovered, effective);
          const resources = local.resources.length
          ...
          for (const op of draft.operations) {
            ...
            if (fingerprint(resource) !== op.schemaHash)
              throw new LocalError(
                "SCHEMA_CHANGED",
            ...
            if (
              snapshot.hash !== op.expectedVersion.snapshotHash ||
              fingerprint(snapshot.record) !== fingerprint(op.before)
            )
              throw new LocalError(
                "PATCH_CONFLICT",
            ...
            for (const [name, value] of Object.entries(op.after)) {
              ...
              if (
                parseValue(field, value === null ? "null" : String(value)) !==
                value
              )
                throw new LocalError(
                  "INVALID_VALUE",
                  "Field value has an invalid type.",
                );
              await validateRelation(db, resources, field, value);
            }
          }
          return LocalProposalSchema.parse({
            id: draft.id,
            ...
            title: draft.title || "Content update",
            createdAt: draft.createdAt,
            operations: draft.operations,
            resources: resources.filter((r) =>
              draft.operations.some((op) => op.resource === r.name),
            ),
          });
        });
        const serialized = JSON.stringify({ server, proposal });
        if (serialized.includes(secret) || serialized.includes(token))
          throw new LocalError(
            "CREDENTIAL_IN_PATCH",
            "A credential was found in patch content; submission is refused.",
          );
    ...
    stdout.write(
      JSON.stringify({
        ...
        reviewUrl: `${server.url}/patches/${patch.id}`,
        warnings,
      }) + "\n",
    );
    return 0;
    ...
    const known =
      error instanceof LocalError || error instanceof PatchctlClientError;
    stderr.write(
      JSON.stringify({
        ok: false,
        error: {
          code: known ? error.code : "INVALID_PATCH",
          message: known
            ? error.message
            : "Patch or local submission state is invalid.",
        },
      }) + "\n",
    );
    return 1;
