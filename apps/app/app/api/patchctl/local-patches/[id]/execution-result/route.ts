...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try { return Response.json(await reportLocalExecution(await readPatchJson(request), await getPatchActor(request), localPatchRepository(), (await context.params).id)); }
