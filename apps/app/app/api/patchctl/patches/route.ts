import { listPatches, preparePatch, PostgresRelationReader } from "@corely/modules-patches";
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json(await preparePatch(await readPatchJson(request), actor, repository, secrets, reader, patches, new PostgresRelationReader()), { status: 201 });
  } catch (error) { return patchProblem(error); }
}
