...
export const PatchProposalInputSchema = z
  .object({
    ...
    agentRunLabel: z.string().max(120).optional(),
    mode: z.enum(["edit", "fill-missing"]).default("edit"),
    translation: z
      .object({ sourceField: fieldName, targetField: fieldName })
      .strict()
      .optional(),
    records: z
      .array(
        z
          .object({
            ...
            changes: z
              ...
              .refine(
                (x) => Object.keys(x).length > 0 && Object.keys(x).length <= 50,
              ),
          })
          .strict(),
      )
  ...
  .strict();
export const ContentQueryInputSchema = z
  .object({
        ...
        z.discriminatedUnion("op", [
          ...
          z
            .object({
              field: fieldName,
              op: z.literal("eq"),
              value: z.string().max(100000).nullable(),
            })
            .strict(),
        ]),
      )
