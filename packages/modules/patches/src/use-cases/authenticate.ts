...
export function verifyIdentity(
  ...
  try {
    ...
    if (
      ...
      (p.nbf !== undefined &&
        (typeof p.nbf !== "number" ||
          !Number.isFinite(p.nbf) ||
          p.nbf > now / 1000)) ||
      typeof p.sub !== "string" ||
      !p.sub ||
      typeof p.tenantId !== "string" ||
      !p.tenantId ||
      (p.kind !== undefined && p.kind !== "human")
    )
      return null;
...
export async function authenticate(
  ...
  if (!actor)
    throw new PatchError(
      401,
      "UNAUTHENTICATED",
      "Credential or active Tenant membership is invalid.",
    );
  ...
  if (requestedTenant && requestedTenant !== actor.tenantId) {
    throw new PatchError(
      403,
      "TENANT_MISMATCH",
      "Requested Tenant does not match the credential.",
    );
  }
  return actor;
}
