...
export async function applyPatch(
  ...
  if (
    ...
    throw new PatchError(
      409,
      "REVISION_MISMATCH",
      "Only the exact approved patch can be applied.",
    );
  if (patch.state === "applied")
    return {
      id,
      state: "applied",
  ...
  if (
    ...
    throw new PatchError(
      409,
      "APPROVAL_REQUIRED",
      "This revision must be approved by a human before apply.",
    );
  ...
  if (
    ...
    throw new PatchError(
      409,
      "SOURCE_CHANGED",
      "Source configuration changed; prepare and review a new patch.",
    );
  for (const record of patch.payload.records)
    for (const [name, value] of Object.entries(record.after)) {
      const field = schema.definition.fields[name];
      if (!field?.editable || !field.readable)
        throw new PatchError(
          409,
          "POLICY_CHANGED",
          "A field is no longer editable.",
        );
      validateFieldValue(value, field, name);
    }
  if (!(await patches.claimApply(patch, source, actor)))
    throw new PatchError(
      409,
      "APPLY_STATE_CHANGED",
      "The source or patch state changed. Refresh before retrying.",
    );
  try {
    ...
    return {
      ...
      affectedRecords: receipt.affectedRecords,
    };
  } catch (error) {
    if (
