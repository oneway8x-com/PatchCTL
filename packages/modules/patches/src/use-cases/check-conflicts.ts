...
export async function checkConflicts(input: unknown, actor: Actor, id: string, patches: PatchRepository, sources: SourceRepository, secrets: SourceSecrets, checker: ConflictChecker) {
  ...
  if (revision !== patch.revision || fingerprint({ tenantId: patch.tenantId, payload: patch.payload }) !== revision)
  ...
  if (fingerprint(schema) !== patch.payload.schemaVersion || fingerprint({ sourceId: source.id, secretRef: source.secretRef, url }) !== patch.payload.sourceFingerprint)
    throw new PatchError(409, "SOURCE_CHANGED", "Source configuration changed. Prepare a new patch.");
  await checker.check(url, schema, actor.tenantId, patch.payload.records);
  return { conflicts: [], checkedAt: new Date().toISOString(), finalCheckRequiredAtApply: true };
}
