...
export interface RelationReader {
  ...
  ): Promise<{ targets: RelationTarget[]; nextCursor: string | null }>;
}
export function validateFieldValue(
  value: ChangeValue,
  field: ContentSchema["fields"][string],
  name: string,
) {
  if (field.type === "text")
    return validateTextValue(
      value,
      { nullable: field.nullable, maxLength: field.maxLength },
      name,
    );
  if (field.type === "timestamp") {
    normalizeTimestamp(value, field.nullable, name);
    return;
  }
  ...
  if (
    ...
    throw new PatchError(
      400,
      "INVALID_ASSIGNMENT",
      `Field ${name} requires a permitted string value or target ID.`,
    );
  if (field.type === "enum" && !field.values?.includes(value))
    throw new PatchError(
      400,
      "INVALID_ENUM",
      `Field ${name} requires a declared enum value.`,
    );
}
