...
export function rowProjection(schema: ContentSchema, fields: string[]) {
  const pairs = fields.flatMap(name => [`'${q(name).slice(1, -1)}'`, schema.fields[name]?.type === "timestamp"
    ? `CASE WHEN isfinite(r.${q(name)}) THEN to_char(r.${q(name)} AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') ELSE r.${q(name)}::text END`
    : `r.${q(name)}${schema.fields[name]?.type === "relation" ? "::text" : ""}`]).join(",");
  return `r.${q(schema.key)}::text AS id, md5(to_jsonb(r)::text || ':' || r.xmin::text) AS version,
    jsonb_build_object(${pairs}) AS values`;
}
export function rowScope(schema: ContentSchema, tenantId: string, values: unknown[]) {
  if (schema.isolation.mode === "dedicated") {
    if (schema.isolation.dedicatedTenantId !== tenantId) throw new PatchError(403, "TENANT_MISMATCH", "Source belongs to another Tenant.");
    return "TRUE";
  }
  values.push(tenantId);
  return `r.${q(schema.isolation.tenantColumn)}::text = $${values.length}`;
}
export async function assertSchema(client: PoolClient, registered: RegisteredSchema) {
...
export class PostgresContentReader implements ContentReader {
  ...
  async query(url: string, registered: RegisteredSchema, tenantId: string, input: ContentQuery): Promise<ContentPage> {
    ...
    for (const field of [...fields, ...input.filters.map(f => f.field)]) {
      if (!Object.hasOwn(schema.fields, field) || !schema.fields[field]?.readable)
        throw new PatchError(400, "UNREADABLE_FIELD", "Only declared readable fields may be selected or filtered.");
    }
    if (fields.length === 0 || new Set(fields).size !== fields.length) throw new PatchError(400, "INVALID_FIELDS", "Select unique readable fields.");
    const values: unknown[] = [];
    ...
    if (input.after !== undefined) {
      ...
      conditions.push(`r.${q(schema.key)}::text COLLATE "C" > $${values.length} COLLATE "C"`);
    }
    ...
    try {
      ...
      const result = await client.query(`SELECT ${rowProjection(schema, fields)} FROM ${tableName(schema)} r
        WHERE ${conditions.join(" AND ")} ORDER BY r.${q(schema.key)}::text COLLATE "C" LIMIT $${values.length}`, values);
      await client.query("COMMIT");
      const records = result.rows.slice(0, input.limit);
      return { records, nextCursor: result.rows.length > input.limit ? records.at(-1)?.id ?? null : null, schemaVersion: fingerprint(registered) };
    } catch (error) { await client.query("ROLLBACK").catch(() => {}); throw error; }
    finally { client.release(); await pool.end(); }
  }
}
