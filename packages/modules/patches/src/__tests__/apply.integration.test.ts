...
describe.skipIf(!url)("atomic approved Postgres application", () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `CREATE TABLE "${namespace}".articles (id text PRIMARY KEY, tenant_id text NOT NULL, title text NOT NULL CHECK(title <> 'Forbidden'), private text)`,
    );
    await pool.query(
  ...
  beforeEach(async () => {
    ...
    await pool.query(
      `INSERT INTO "${namespace}".articles VALUES ('1',$1,'First','secret'),('2',$1,'Second','secret'),('other','other','Untouched','secret')`,
      [tenantId],
    );
    const rows = await new PostgresContentReader().snapshots(
      url!,
      schema,
      tenantId,
      ["1", "2"],
    ...
    patch = {
      ...
      payload: {
        ...
        sourceFingerprint: fingerprint({ sourceId, secretRef: "local", url }),
        reason: "Correct text",
        ...
        records: rows.map((r) => ({
          id: r.id,
          version: r.version,
          before: { title: r.values.title! },
          after: { title: `New ${r.id}` },
        })),
      },
    };
  ...
  it("writes approved fields atomically and records attribution without touching other tenants", async () => {
    ...
    expect(
      (
        await pool.query(
          `SELECT title,private FROM "${namespace}".articles ORDER BY id`,
        )
      ).rows,
    ).toEqual([
      { title: "New 1", private: "secret" },
      { title: "New 2", private: "secret" },
      { title: "Untouched", private: "secret" },
    ]);
    expect(
      (
        await pool.query(
  ...
  it("rolls back earlier writes if a later row violates a constraint", async () => {
    ...
    expect(
      (
        await pool.query(
          `SELECT title FROM "${namespace}".articles WHERE tenant_id=$1 ORDER BY id`,
          [tenantId],
        )
      ).rows.map((r) => r.title),
    ...
    expect(
      (
        await pool.query(
          "SELECT 1 FROM public.patchctl_apply_receipts WHERE tenant_id=$1 AND patch_id=$2",
          [tenantId, patch.id],
        )
      ).rowCount,
    ).toBe(0);
  ...
  it("deduplicates concurrent calls and retries without another write", async () => {
    ...
    const versions = await new PostgresContentReader().snapshots(
      url!,
      schema,
      tenantId,
      ["1", "2"],
    ...
    expect(
      await new PostgresContentReader().snapshots(url!, schema, tenantId, [
        "1",
        "2",
      ]),
    ).toEqual(versions);
  ...
  it("blocks changed records with no partial update", async () => {
    await pool.query(
      `UPDATE "${namespace}".articles SET private='edited externally' WHERE id='2'`,
    );
    await expect(
      writer.apply(url!, schema, patch, actor),
  ...
  it("recovers after target commit succeeds but metadata persistence fails", async () => {
    const repository = {
      ...
      finishApply: vi
        .fn()
        .mockRejectedValueOnce(new Error("metadata connection lost"))
    ...
    ).rejects.toThrow("metadata connection lost");
