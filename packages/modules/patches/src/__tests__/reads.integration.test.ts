...
describe.skipIf(!url)("scoped Postgres content reads", () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `CREATE TABLE "${namespace}".articles (id text PRIMARY KEY, tenant_id text NOT NULL, title text NOT NULL, summary_en text, private text)`,
    );
    for (let i = 0; i < 55; i++)
      await pool.query(
        `INSERT INTO "${namespace}".articles VALUES ($1,'t',$2,$3,'secret')`,
        [
    ...
    await pool.query(
      `INSERT INTO "${namespace}".articles VALUES ('other','other','Other tenant',NULL,'secret')`,
    );
    const definition = contentSchemaInput.parse({
      ...
      fields: {
        ...
        private: { type: "text", readable: false, editable: false },
      },
    });
    schema = {
      definition,
  ...
  it("paginates beyond 50 records without exposing another Tenant or hidden columns", async () => {
    const first = await reader.query(
      ...
      schema,
      "t",
      queryInput.parse({ limit: 50 }),
    );
    const second = await reader.query(
      url!,
      schema,
      "t",
      queryInput.parse({ after: first.nextCursor }),
    );
  ...
  it("finds null and whitespace values and applies equality filters without SQL injection", async () => {
    const missing = await reader.query(
      url!,
      schema,
      "t",
      queryInput.parse({ filters: [{ field: "summary_en", op: "missing" }] }),
    );
    expect(missing.records).toHaveLength(37);
    const exact = await reader.query(
      url!,
      schema,
      "t",
      queryInput.parse({
        filters: [{ field: "title", op: "eq", value: "Article 0" }],
      }),
    );
    ...
    expect(
      (
        await reader.query(
          ...
          "t",
          queryInput.parse({
            filters: [{ field: "title", op: "eq", value: "' OR 1=1 --" }],
          }),
        )
      ).records,
    ).toEqual([]);
  ...
  it("denies hidden field reads and changes version after edits to unrelated fields", async () => {
    await expect(
      reader.query(
        ...
        schema,
        "t",
        queryInput.parse({ fields: ["private"] }),
      ),
    ).rejects.toMatchObject({ code: "UNREADABLE_FIELD" });
    const before = await reader.query(
      url!,
      schema,
      "t",
      queryInput.parse({ limit: 1 }),
    );
    await pool.query(
      `UPDATE "${namespace}".articles SET private='changed' WHERE id='000'`,
    );
