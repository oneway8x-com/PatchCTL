...
function request(credential?: string, tenant?: string) {
  return new Request("http://localhost/api/patchctl/me", {
    headers: {
...
describe("verified PatchCTL access", () => {
  it("resolves a verified human and checks current membership", async () => {
    ...
    expect(repo.human).toHaveBeenCalledWith({
      userId: "user",
      tenantId: "tenant",
    });
  });
  it.each([
    ...
    token({ exp: null }),
    token({ tenantId: null }),
    token({ nbf: Date.now() / 1000 + 500 }),
    token({ kind: "agent" }),
    token({}, { alg: "none", typ: "JWT" }),
  ])(
    "rejects invalid credentials without a repository lookup",
    async (credential) => {
      const repo = { human: vi.fn(), agent: vi.fn() };
      await expect(
        authenticate(request(credential, "tenant"), repo, secret),
  ...
  it("rejects a forged Tenant header even with a valid credential", async () => {
    await expect(
      authenticate(
        request(token(), "other"),
        { human: async () => human, agent: async () => null },
        secret,
      ),
  ...
  it("denies removed memberships and modified signatures", async () => {
    await expect(
      authenticate(
        request(token()),
        { human: async () => null, agent: async () => null },
        secret,
      ),
    ).rejects.toMatchObject({ status: 401 });
  ...
  it("hashes an agent secret and never treats it as a human session", async () => {
    const actor: Actor = {
      ...
      connectionIds: ["source"],
    };
    const repo = { human: vi.fn(), agent: vi.fn().mockResolvedValue(actor) };
    expect(
      await authenticate(request("pct_" + "a".repeat(32)), repo, secret),
    ).toEqual(actor);
  ...
  it.each(["review", "apply", "configure"] as const)(
    "denies agent %s even with an accidentally granted permission",
    (permission) => {
      expect(() =>
        authorize(
          { ...human, kind: "agent", permissions: [permission] },
          permission,
        ),
      ).toThrow();
    },
  ...
  it("denies cross-Tenant resources and insufficient human permissions", () => {
    ...
    expect(() =>
      authorize({ ...human, permissions: ["read"] }, "review"),
    ).toThrow();
  });
});
...
describe("identity repository permission policy", () => {
  it("applies explicit DENY even to an administrator", async () => {
    ...
    expect(db.membership.findFirst.mock.calls[0]?.[0].where).toMatchObject({
      tenantId: "tenant",
      user: { status: "ACTIVE" },
  ...
  it("requires agent ownership, expiry/revocation filtering and active owner membership", async () => {
    ...
    expect(
      await new PrismaAccessRepository(db as never).agent("hash"),
    ).toBeNull();
    expect(db.apiKey.findFirst.mock.calls[0]?.[0].where).toMatchObject({
      keyHash: "hash",
      revokedAt: null,
