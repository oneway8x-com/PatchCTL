...
import { contentSchemaInput, fingerprint, quoteIdentifier } from "../content-schema";
export const articleSchema = { namespace: "public", table: "articles", key: "id", isolation: { mode: "row", tenantColumn: "tenant_id" }, fields: {
  title: { type: "text", readable: true, editable: true }, summary_en: { type: "text", readable: true, editable: true, nullable: true, locale: "en" },
...
describe("declared content schema", () => {
  it("requires explicit Tenant isolation and safe identifiers", () => {
    ...
    expect(() => contentSchemaInput.parse({ ...articleSchema, isolation: undefined })).toThrow();
    expect(() => quoteIdentifier('articles; DROP TABLE users')).toThrow();
    expect(() => contentSchemaInput.parse({ ...articleSchema, arbitrarySQL: "select 1" })).toThrow();
  });
  it("protects keys, Tenant IDs, unreadable and unsupported fields", () => {
    ...
    ]) expect(() => contentSchemaInput.parse({ ...articleSchema, fields })).toThrow();
  });
