...
describe.skipIf(!url)(
  "Prisma identity, transitions and atomic audit persistence",
  () => {
    ...
    const sources = new PrismaSourceRepository(db),
      patches = new PrismaPatchRepository(db),
      access = new PrismaAccessRepository(db);
    const secrets = new EnvironmentSourceSecrets(
      JSON.stringify({ local: { tenantId, url } }),
    );
    ...
    beforeAll(async () => {
      ...
      await pool.query(
        `CREATE TABLE "${namespace}".articles (id text PRIMARY KEY, tenant_id text NOT NULL, title text NOT NULL)`,
      );
      await pool.query(
      ...
      await db.tenant.create({
        data: { id: tenantId, name: "Test", slug: tenantId },
      });
      ...
      await db.role.create({
        data: { id: roleId, tenantId, name: "Test admin", systemKey: "ADMIN" },
      });
      ...
      await db.apiKey.create({
        data: {
          ...
          scopes: ["read", "propose", "review", "apply"],
          connectionIds: [sourceId],
          expiresAt: new Date(Date.now() + 3600000),
        },
      });
      actor = (await access.human({ userId, tenantId }))!;
      agent = (await access.agent(
        createHash("sha256").update(keyId).digest("hex"),
      ))!;
    });
    beforeEach(async () => {
      await pool.query(`DELETE FROM "${namespace}".articles`);
      await pool.query(
        `INSERT INTO "${namespace}".articles VALUES ('1',$1,'Before')`,
        [tenantId],
      );
    });
    afterAll(async () => {
      ...
      await db.auditLog.deleteMany({
        where: { tenantId, entity: "ContentPatch" },
      });
      ...
      return preparePatch(
        {
          ...
          reason: "Fix",
          records: [
            { id: "1", version: rows[0]!.version, changes: { title: "After" } },
          ],
        },
        agent,
        sources,
    ...
    it("derives real membership and clamps agent scopes; removed memberships are denied", async () => {
      ...
      expect(
        await access.agent(createHash("sha256").update(keyId).digest("hex")),
      ).toBeNull();
      await db.apiKey.update({
        where: { id: keyId },
        data: { revokedAt: null },
    ...
    it("commits exactly one competing review decision and event", async () => {
      ...
      const results = await Promise.allSettled(
        ["approved", "rejected"].map((decision) =>
          decidePatch(
            { revision: proposal.revision, decision },
            actor,
            proposal.id,
            patches,
            sources,
    ...
    it("persists full provenance and one successful apply event, even after replay and record deletion", async () => {
      ...
      const history = await patchHistory(
        {},
        actor,
        proposal.id,
        patches,
      ...
      expect(history.events[0]?.actor).toMatchObject({
        id: keyId,
        kind: "agent",
      });
      expect(history.events[3]?.details).toMatchObject({
        reviewerId: userId,
        appliedBy: userId,
        records: [{ before: { title: "Before" }, after: { title: "After" } }],
      });
      const first = await patchHistory(
        { limit: 2 },
        actor,
        proposal.id,
        patches,
      ...
      const second = await patchHistory(
      ...
      expect(second.events.map((e) => e.action)).toEqual([
        "apply-attempt",
        "applied",
      ]);
      await expect(
