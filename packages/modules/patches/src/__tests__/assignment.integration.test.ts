...
describe.skipIf(!url)("scoped enum and relation assignments", () => {
  const tenantId = randomUUID(),
    sourceId = randomUUID(),
    namespace = `pct_assign_${tenantId.replaceAll("-", "")}`;
  ...
  const reader = new PostgresContentReader(),
    relations = new PostgresRelationReader(),
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `CREATE TABLE "${namespace}".categories (id text PRIMARY KEY, tenant_id text NOT NULL, label text NOT NULL)`,
    );
    await pool.query(
      `CREATE TABLE "${namespace}".articles (id text PRIMARY KEY, tenant_id text NOT NULL, placement text NOT NULL, category_id text REFERENCES "${namespace}".categories(id))`,
    );
    await pool.query(
  ...
  beforeEach(async () => {
    ...
    await pool.query(`DELETE FROM "${namespace}".categories`);
    await pool.query(
      `INSERT INTO "${namespace}".categories VALUES ('old',$1,'Old category'),('new',$1,'New category'),('private','other','Secret label')`,
      [tenantId],
    );
    await pool.query(
      `INSERT INTO "${namespace}".articles VALUES ('1',$1,'HOME','old'),('2',$1,'HOME',NULL)`,
      [tenantId],
    );
  });
  afterAll(async () => {
  ...
  it("discovers only permitted targets with bounded pagination", async () => {
    ...
    const next = await readRelationTargets(
    ...
    expect(next.targets.map((t) => t.id)).toEqual(["old"]);
  ...
  it("applies a bulk EARN_TOP/category assignment with immutable IDs and labels in its receipt", async () => {
    ...
    expect(patch.payload.records[0]?.relations?.category_id).toMatchObject({
      before: { id: "old", label: "Old category" },
      after: { id: "new", label: "New category" },
    });
    ...
    expect(
      (
        await pool.query(
          `SELECT placement,category_id FROM "${namespace}".articles`,
        )
      ).rows,
    ).toEqual([
      { placement: "EARN_TOP", category_id: "new" },
      { placement: "EARN_TOP", category_id: "new" },
    ]);
  });
  it.each([
    ...
    { category_id: 12 },
    { placement: null },
  ])("rejects invalid or out-of-Tenant assignments", async (changes) => {
    await expect(prepare(changes)).rejects.toMatchObject({ status: 400 });
  });
  it.each(["delete", "move", "rename"])(
    "blocks the whole patch when a target is changed: %s",
    async (action) => {
      ...
      if (action === "delete")
        await pool.query(
          `DELETE FROM "${namespace}".categories WHERE id='new'`,
        );
      if (action === "move")
        await pool.query(
          `UPDATE "${namespace}".categories SET tenant_id='other' WHERE id='new'`,
        );
      if (action === "rename")
        await pool.query(
          `UPDATE "${namespace}".categories SET label='Changed label' WHERE id='new'`,
        );
      await expect(
        writer.apply(url!, schema, patch, actor),
      ).rejects.toMatchObject({ code: "RELATION_CONFLICT" });
      expect(
        (
          await pool.query(`SELECT placement FROM "${namespace}".articles`)
        ).rows.every((r) => r.placement === "HOME"),
      ).toBe(true);
    },
  );
  it("requires a real foreign key, not just a configured relation label", async () => {
    const definition = structuredClone(schema.definition);
    definition.fields.category_id!.relation!.table = "articles";
    await expect(
      new PostgresSchemaInspector().inspect(url!, definition),
