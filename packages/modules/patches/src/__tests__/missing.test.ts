...
function setup(target: string | null) {
  const input = {
    ...
    reason: "Add English translation",
    mode: "fill-missing",
    ...
    records: [
      {
        id: "1",
        version: "b".repeat(32),
        changes: { summary_en: "Hello, world!" },
      },
    ],
  };
  const sources = {
    ...
    snapshots: vi.fn(async () => [
      {
        id: "1",
        version: "b".repeat(32),
        values: { summary_fr: "Bonjour, le monde !", summary_en: target },
      },
    ]),
  };
...
describe("missing content and translation proposals", () => {
  it.each([null, "", " \t\n", "\u00a0\u2003\ufeff"])(
    "fills a missing target while preserving source locale",
    async (target) => {
  ...
  it("rejects already populated targets and empty replacements", async () => {
    ...
    await expect(populated.run()).rejects.toMatchObject({
      code: "TARGET_NOT_MISSING",
    });
    expect(populated.patches.create).not.toHaveBeenCalled();
    const empty = setup(null);
    empty.input.records[0]!.changes.summary_en = " ";
    await expect(empty.run()).rejects.toMatchObject({
      code: "TARGET_NOT_MISSING",
    });
  });
  it("rejects source changes after the agent read and invalid locale mappings", async () => {
    const stale = setup(null);
    stale.input.records[0]!.version = "c".repeat(32);
    await expect(stale.run()).rejects.toMatchObject({ code: "STALE_RECORD" });
    const invalid = setup(null);
    invalid.input.translation.sourceField = "summary_en";
    await expect(invalid.run()).rejects.toMatchObject({
