...
describe.skipIf(!url)("isolated Postgres schema and connection", () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `CREATE TABLE "${testNamespace}".articles (id text PRIMARY KEY, tenant_id text NOT NULL, title text NOT NULL, summary_en text)`,
    );
  });
  afterAll(async () => {
  ...
  it("connects and introspects the declared primary key and fields", async () => {
    ...
    expect(
      await new PostgresSchemaInspector().inspect(
        url!,
        contentSchemaInput.parse(input),
      ),
    ).toMatch(/^[a-f0-9]{64}$/);
  });
  it("rejects undeclared key identity and missing columns", async () => {
    ...
    await expect(
      inspector.inspect(
        url!,
        contentSchemaInput.parse({ ...input, key: "tenant_id" }),
      ),
    ).rejects.toMatchObject({ code: "INVALID_SCHEMA" });
    await expect(
      inspector.inspect(
        url!,
        contentSchemaInput.parse({
          ...input,
          fields: { missing: { type: "text", readable: true, editable: true } },
        }),
      ),
    ).rejects.toMatchObject({ code: "INVALID_SCHEMA" });
  });
  it("changes fingerprint when the live schema changes", async () => {
    ...
    await pool.query(
      `ALTER TABLE "${testNamespace}".articles ADD COLUMN extra text`,
    );
    expect(await inspector.inspect(url!, schema)).not.toBe(before);
  });
  it("redacts credentials from connection failures", async () => {
    await expect(
      new PostgresProbe().test(
        "postgresql://private:secret@127.0.0.1:1/missing",
      ),
    ).rejects.toMatchObject({
      message: "Could not connect to the content source.",
    });
  });
  it("rejects user triggers and row-security policies instead of assuming side effects are safe", async () => {
    const inspector = new PostgresSchemaInspector(),
      schema = contentSchemaInput.parse(input);
    await pool.query(
      `CREATE FUNCTION "${testNamespace}".touch() RETURNS trigger LANGUAGE plpgsql AS 'BEGIN RETURN NEW; END'`,
    );
    await pool.query(
      `CREATE TRIGGER touch BEFORE UPDATE ON "${testNamespace}".articles FOR EACH ROW EXECUTE FUNCTION "${testNamespace}".touch()`,
    );
    await expect(inspector.inspect(url!, schema)).rejects.toMatchObject({
      code: "UNSUPPORTED_TABLE_BEHAVIOR",
    });
    await pool.query(`DROP TRIGGER touch ON "${testNamespace}".articles`);
    await pool.query(
      `ALTER TABLE "${testNamespace}".articles ENABLE ROW LEVEL SECURITY`,
    );
    await expect(inspector.inspect(url!, schema)).rejects.toMatchObject({
      code: "UNSUPPORTED_TABLE_BEHAVIOR",
    });
    await pool.query(
      `ALTER TABLE "${testNamespace}".articles DISABLE ROW LEVEL SECURITY`,
    );
  });
});
