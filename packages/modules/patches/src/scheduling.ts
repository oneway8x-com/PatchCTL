...
export function normalizeTimestamp(
  ...
  if (
    ...
    throw new PatchError(
      400,
      "INVALID_TIMESTAMP",
      `${name} requires a valid ISO timestamp with Z or an explicit offset and at most millisecond precision.`,
    );
  ...
  if (!/^\d{4}-/.test(normalized))
    throw new PatchError(
      400,
      "INVALID_TIMESTAMP",
      `${name} must be within years 0001–9999.`,
    );
...
export function validateSchedules(
  ...
  for (const pair of schema.schedules) {
    if (
      !Object.hasOwn(changes, pair.startsAt) &&
      !Object.hasOwn(changes, pair.endsAt)
    )
      continue;
    const values = { ...current, ...changes };
    const start = normalizeTimestamp(
      values[pair.startsAt]!,
      schema.fields[pair.startsAt]!.nullable,
      pair.startsAt,
    );
    const end = normalizeTimestamp(
      values[pair.endsAt]!,
      schema.fields[pair.endsAt]!.nullable,
      pair.endsAt,
    );
    if (start !== null && end !== null && start >= end)
      throw new PatchError(
        ...
        `${pair.startsAt} must be earlier than ${pair.endsAt}.`,
      );
  }
}
