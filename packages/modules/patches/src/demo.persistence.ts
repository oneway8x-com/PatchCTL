...
export async function seedDemo(url: string) {
  ...
  try {
    ...
    await pool.query(`INSERT INTO "${namespace}".categories VALUES ('news',$1,'News')`, [tenantId]);
    await pool.query(`CREATE TABLE "${namespace}".articles (id text PRIMARY KEY, tenant_id text NOT NULL, title text NOT NULL, body text NOT NULL, summary_en text, summary_fr text, placement text NOT NULL DEFAULT 'HOME', category_id text REFERENCES "${namespace}".categories(id))`);
    ...
    for (let n = 1; n <= 10; n++) await pool.query(`INSERT INTO "${namespace}".articles (id,tenant_id,title,body,summary_en) VALUES ($1,$2,$3,$4,$5)`,
      [String(n).padStart(2, "0"), tenantId, `Article ${n}`, `Article ${n} explains how reviewed content changes protect a publication.`, [null, "", " \t "][n % 3]]);
    await pool.query(`INSERT INTO "${namespace}".articles (id,tenant_id,title,body,summary_en) VALUES ('11',$1,'Control','Already summarized','Keep this summary'),('12',$2,'Other Tenant','Private content',NULL)`, [tenantId, randomUUID()]);
    ...
    const definition = contentSchemaInput.parse({ namespace, table: "articles", key: "id", isolation: { mode: "row", tenantColumn: "tenant_id" },
      fields: { title: { type: "text", readable: true, editable: true }, body: { type: "text", readable: true, editable: false }, summary_en: { type: "text", readable: true, editable: true, nullable: true, locale: "en" },
        summary_fr: { type: "text", readable: true, editable: false, nullable: true, locale: "fr" }, placement: { type: "enum", values: ["HOME", "EARN_TOP"], readable: true, editable: true },
