...
export async function seedDemo(url: string) {
  ...
  if (
    ...
    !/_(test|demo)$/.test(target.pathname)
  )
    throw new Error(
      "Demo requires a loopback database ending in _test or _demo.",
    );
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO "${namespace}".categories VALUES ('news',$1,'News')`,
      [tenantId],
    );
    await pool.query(
      `CREATE TABLE "${namespace}".articles (id text PRIMARY KEY, tenant_id text NOT NULL, title text NOT NULL, body text NOT NULL, summary_en text, summary_fr text, placement text NOT NULL DEFAULT 'HOME', category_id text REFERENCES "${namespace}".categories(id))`,
    );
    await pool.query(
    ...
    for (let n = 1; n <= 10; n++)
      await pool.query(
        `INSERT INTO "${namespace}".articles (id,tenant_id,title,body,summary_en) VALUES ($1,$2,$3,$4,$5)`,
        [
          String(n).padStart(2, "0"),
          tenantId,
          `Article ${n}`,
          `Article ${n} explains how reviewed content changes protect a publication.`,
          [null, "", " \t "][n % 3],
        ],
      );
    await pool.query(
      `INSERT INTO "${namespace}".articles (id,tenant_id,title,body,summary_en) VALUES ('11',$1,'Control','Already summarized','Keep this summary'),('12',$2,'Other Tenant','Private content',NULL)`,
      [tenantId, randomUUID()],
    );
    await db.tenant.create({
      data: { id: tenantId, name: "PatchCTL demo", slug: tenantId },
    });
    await db.user.create({
      data: {
        id: userId,
        email: `${userId}@example.test`,
        passwordHash: "unused-demo-only",
      },
    });
    const role = await db.role.create({
      data: { tenantId, name: "Demo reviewer", systemKey: "ADMIN" },
    });
    ...
    await new PrismaSourceRepository(db).save(
      {
        id: sourceId,
        tenantId,
        name: "Demo articles",
    ...
    const parts = [
      { alg: "HS256", typ: "JWT" },
      { sub: userId, tenantId, exp: Math.floor(Date.now() / 1000) + 3600 },
