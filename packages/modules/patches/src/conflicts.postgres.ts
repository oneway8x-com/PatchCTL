...
export async function lockAndValidateRecords(
  client: PoolClient,
  ...
  await client.query(
    `LOCK TABLE ${tableName(schema.definition)} IN ROW SHARE MODE`,
  );
  const targets = [
    ...new Set(
      Object.values(schema.definition.fields)
        .filter((field) => field.type === "relation" && field.relation)
        .map((field) => tableName(field.relation!)),
    ),
  ].sort();
  for (const target of targets)
    await client.query(`LOCK TABLE ${target} IN ROW SHARE MODE`);
  ...
  const conflicts = records.flatMap((record) => {
    ...
    if (
      ...
      Object.entries(record.before).every(
        ([field, value]) => row.values[field] === value,
      )
    )
      return [];
    return [
      {
        id: record.id,
        code: row ? "CHANGED" : "UNAVAILABLE",
        before: record.before,
        proposed: record.after,
        current: row
          ? Object.fromEntries(
  ...
  if (conflicts.length)
    throw new PatchError(
      409,
      "RECORD_CONFLICT",
      "Records changed after preparation. No changes were applied; prepare and review a new patch.",
      { conflicts },
    );
  return current;
}
...
export class PostgresConflictChecker implements ConflictChecker {
  async check(
    ...
    try {
      ...
      await lockAndValidateRecords(client, schema, tenantId, records);
    } finally {
