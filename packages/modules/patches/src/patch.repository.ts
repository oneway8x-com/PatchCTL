...
export class PrismaPatchRepository
  implements PatchApplyRepository, PatchAuditRepository
{
  ...
  private map(row: ContentPatch): Patch {
    return {
      ...
      payload: row.payloadJson as unknown as PatchPayload,
    ...
    await this.db.$transaction(async (tx) => {
      await tx.contentPatch.create({
        data: {
          ...
          state: patch.state,
          creatorId: patch.payload.creator.id,
          payloadJson: patch.payload as unknown as Prisma.InputJsonValue,
        },
      });
      await tx.auditLog.create({
        data: {
          ...
          action: "prepared",
          actorUserId:
            patch.payload.creator.kind === "human"
              ? patch.payload.creator.id
              : null,
          details: JSON.stringify({
            actor: patch.payload.creator,
            revision: patch.revision,
            records: patch.payload.records,
            reason: patch.payload.reason,
          }),
        },
      });
    });
  ...
  async find(tenantId: string, id: string) {
    const row = await this.db.contentPatch.findFirst({
      where: { id, tenantId },
    });
  ...
  async decide(
    ...
    return this.db.$transaction(async (tx) => {
      const result = await tx.contentPatch.updateMany({
        where: { id, tenantId, revision, state: "pending" },
        data: {
          state: decision,
          reviewerId,
          reviewedAt: new Date(),
      ...
      if (result.count === 1)
        await tx.auditLog.create({
          data: {
            ...
            details: JSON.stringify({
              actor: { id: reviewerId, kind: "human" },
              revision,
    ...
    return this.db.$transaction(async (tx) => {
      ...
      const current = await tx.integrationConnection.findFirst({
        where: {
          id: source.id,
          tenantId: patch.tenantId,
          providerKey: "patchctl.postgres",
          status: "active",
        },
      });
      if (
        !current ||
        canonical(current.configJson) !==
          canonical({ secretRef: source.secretRef, schema: source.schema })
      )
        return false;
      const result = await tx.contentPatch.updateMany({
        where: {
          ...
          tenantId: patch.tenantId,
          revision: patch.revision,
          state: { in: ["approved", "applying"] },
          reviewerId: { not: null },
        },
        data: { state: "applying", failureCode: null },
      });
      if (result.count === 1)
        await tx.auditLog.create({
          data: {
  ...
  async finishApply(patch: Patch, receipt: ApplyReceipt) {
    await this.db.$transaction(async (tx) => {
      const result = await tx.contentPatch.updateMany({
        ...
        data: {
          state: "applied",
          appliedAt: new Date(receipt.appliedAt),
          failureCode: null,
        },
      });
      if (result.count === 1)
        await tx.auditLog.create({
          data: {
            ...
            entityId: patch.id,
            action: "applied",
            actorUserId: receipt.appliedBy,
            createdAt: new Date(receipt.appliedAt),
            details: JSON.stringify({
              ...receipt,
              actor: { id: receipt.appliedBy, kind: "human" },
            }),
          },
        });
    });
  ...
  async list(
    ...
    const rows = await this.db.contentPatch.findMany({
      where: {
        tenantId,
        ...(connectionIds !== null ? { sourceId: { in: connectionIds } } : {}),
        ...(after ? { id: { gt: after } } : {}),
      },
      orderBy: { id: "asc" },
      take: limit,
  ...
  async history(
    ...
    const cursor = after
      ? await this.db.auditLog.findFirst({ where: { ...base, id: after } })
      : null;
