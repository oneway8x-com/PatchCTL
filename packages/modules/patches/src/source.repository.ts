...
export class PrismaSourceRepository implements SourceRepository {
  ...
  async find(tenantId: string, id: string) {
    const row = await this.db.integrationConnection.findFirst({
      where: { id, tenantId, providerKey, status: "active" },
    });
    return row ? this.map(row) : null;
  }
  async list(tenantId: string) {
    return (
      await this.db.integrationConnection.findMany({
        where: { tenantId, providerKey, status: "active" },
        orderBy: { id: "asc" },
      })
    ...
    if (create) {
      ...
      await this.db.$transaction(async (tx) => {
        ...
        const result = await tx.integrationConnection.updateMany({
          where: { id: source.id, tenantId: source.tenantId, providerKey },
          data: { displayName: source.name, configJson },
        });
        if (result.count !== 1)
