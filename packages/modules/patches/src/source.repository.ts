...
export class PrismaSourceRepository implements SourceRepository {
  ...
  async save(source: Source, create: boolean) {
    ...
    if (create) {
      ...
      const result = await this.db.integrationConnection.updateMany({ where: { id: source.id, tenantId: source.tenantId, providerKey }, data: { displayName: source.name, configJson } });
